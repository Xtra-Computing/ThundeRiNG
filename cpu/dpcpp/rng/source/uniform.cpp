...
int check_statistics(std::vector <Type, mkl_allocator<Type, 64>>& r, Type a, Type b);
...
int run_uniform_example(const sycl::device &dev) {
    ...
    std::vector <Type, mkl_allocator<Type, 64>> r;
    r.resize(N);
    ...
    //
    {
        sycl::buffer<Type, 1> r_buffer(r.data(), sycl::range<1>(r.size()));
        ...
        auto r_accessor = r_buffer.template get_access<sycl::access::mode::read>();
...
int main() {
    ...
    oneapi::mkl::rng::uniform<uint32_t> distr;

    {
        // buffer for random numbers
        sycl::buffer<uint32_t, 1> r_buf(r.data(), r.size());
        // perform generation
    ...
    for (auto it = list_of_devices.begin(); it != list_of_devices.end(); ++it) {
        ...
        get_sycl_device(my_dev, my_dev_is_found, *it);
            ...
            if(my_dev_is_found) {
                ...
                std::cout << "\tRunning with unsigned integer data type:" << std::endl;
