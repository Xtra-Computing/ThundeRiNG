...
int main(int argc, char **argv)
{
    ...
    for (i = 0; i < GPU_N; i++)
    {
        ...
        printf("optionSolver[i].optionCount %d \n",optionSolver[i].optionCount);
    ...
    if (!use_threads || bqatest)
    {
        ...
        printf("Options per sec.: %f\n", OPT_N / (time * 0.001));
        printf("%d - %d%d cTotal time (ms.):  %f\n",__TEST_S__,__TEST_THUNDERING_TEST__,__TEST_STATE_SHARE__,time);
